...
설계 근거는 [`vault-architecture.md`](vault-architecture.md),
Vault 자체를 고치는 절차는 [`runbook-vault.md`](runbook-vault.md).

---

## 먼저 알아야 할 것 셋

### ① 경로를 먼저 정한다. 값이 아니라 **누가 읽느냐**가 경로를 정한다
...
✗ secret/ssh/keys                   ← 한 경로에 뭉치면
    authorized_keys = ...              공개키를 읽는 terraform 이
...
새 시크릿을 넣기 전에 표를 한 줄 채워본다. 칸이 안 채워지면 경로가 잘못된 것이다.

| 값 | 누가 읽나 | 누가 읽으면 안 되나 | 경로 |
|---|---|---|---|

### ② 값은 사람이 넣는다. terraform 이 넣지 않는다

`vault_kv_secret_v2` **리소스**가 있지만 쓰지 않는다. 그 값이 tfstate 에
평문으로 들어간다 — 시크릿을 한 곳에 모으려는 작업이 새 사본을 만든다.
...
짜면, 그 폴백이 동작하는 동안 아무도 Vault 경로가 깨진 걸 모른다. 그리고
평문 경로가 영원히 안 지워진다.
...
예시로 `secret/aws/network` 에 `terraform/aws/network` 의 tfvars 값을
넣는다고 하자. (실제로 필요한 작업이다 — 그 tfvars 가 리포에 없어서 지금
그 스택을 apply 할 수 없다.)
...
읽는 주체 — 사람(`pneuma`)뿐. terraform 을 사람이 돌리므로 AppRole 이 필요 없다.
**AppRole 이 필요한 것은 CI 가 붙을 때다.**
...
# 없으면 "unsupported path" 로 죽고 원인이 권한이라는 게 안 드러난다.
...
> **`secret/<경로>` 는 CLI 가 쓰는 표기이고 정책에는 그렇게 안 적는다.**
...
**경계 확인을 생략하지 않는다.** 정책을 넓게 쓴 실수는 아무 에러도 내지 않는다.
...
실수로 state 에 새는 경로가 없다. 자원 인자로 넣어야 하는 값이면 그건
`data` 를 써야 하고, **그러면 state 에 남는다는 뜻이므로 비밀이면 안 된다.**

값이 비밀인데 자원 인자로 들어가야 한다면 설계가 잘못된 것이다. 그 값을
terraform 이 아니라 다른 곳에서 주입할 방법을 찾는다(cloud-init 이 부팅 시
Vault 에서 직접 읽게 하는 식).
...
- 반환값의 `.secret` 이 실제 필드 딕셔너리다 (`.data` 아니다)
...
`ssh-add -t` 로 수명을 주는 것이 핵심이다. **Vault 토큰 TTL(8h)과 같은 값을
쓴다.** 다르면 한쪽이 살아 있는데 다른 쪽이 죽어 원인을 헷갈린다.

### K8s (후속)

아직 없다. External Secrets Operator 로 `secret/` 를 K8s Secret 으로 동기화할
예정이고, 그건 `repos/02-k8s-manifests` 가 갖는다. 클러스터 안에서는 이
런북이 아니라 그쪽을 본다.
...
**옛 토큰을 Proxmox 에서 지우는 것은 확인 뒤에 한다.** 순서를 바꾸면 새 값이
틀렸을 때 되돌릴 수단이 없다.
...
reusable + 해당 태그 부여 권한으로 발급한다. 노드용은 ephemeral 로 받는 것을
권한다 — VM 을 다시 만들 때 같은 이름의 죽은 디바이스가 남으면 Tailscale 이
뒤에 `-1` 을 붙이고 메시 이름이 달라진다.
...
> 감사 없이 시크릿을 내주지 않는다. 단일 노드라 우회 경로가 없으므로
> 디스크가 차지 않는지 사람이 본다. 롤이 logrotate 를 넣지만 그것만으로는
> 부족하다.
