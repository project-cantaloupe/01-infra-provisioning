...
# 워커 노드를 클러스터에 조인시킨다.
#
# 조인 자체는 ansible 롤(kubeadm-worker)이 한다. 이 스크립트는 그 앞뒤를 맡는다.
#   - 컨트롤플레인이 준 join 명령을 통째로 받아 3개 값으로 쪼갠다
#   - 조인 전에 실패할 조건들을 먼저 확인한다 (여기서 걸리면 조인은 반드시 실패한다)
#   - 조인 후 실제로 붙었는지 검증한다
#
# 왜 사전 점검이 필요한가 — kubeadm join 은 실패해도 원인을 잘 안 알려준다.
# 버전 불일치, 토큰 만료, 도달 불가가 전부 비슷한 타임아웃으로 나타난다.
# 미리 갈라두면 어느 쪽인지 즉시 안다.
#
# 사용법:
...
#
# 토큰은 화면에도 로그에도 남기지 않는다. 환경변수로만 ansible 에 넘긴다.
