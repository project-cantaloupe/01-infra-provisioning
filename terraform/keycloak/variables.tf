...
variable "secops_username" {
  description = "보안 담당 계정의 username. **K8s RBAC 주체 이름이 여기서 나온다** — oidc:<이 값>"
  type        = string
  default     = "secops"

  validation {
    # platform_admin_username 과 같은 규칙이다. 대문자나 점이 들어가면
    # RBAC 매니페스트에서 틀리기 쉽고, 틀려도 에러가 아니라 **권한이
    # 조용히 안 붙는** 형태로 나타난다.
    condition     = can(regex("^[a-z0-9][a-z0-9-]*$", var.secops_username))
    error_message = "username 은 소문자·숫자·하이픈만 쓴다."
  }
}

variable "secops_email" {
  description = "보안 담당 계정의 이메일. 선언적 사용자 프로필이 필수로 요구한다"
  type        = string
}

variable "secops_first_name" {
  description = "보안 담당 계정의 이름"
  type        = string
}

variable "secops_last_name" {
  description = "보안 담당 계정의 성"
  type        = string
}
